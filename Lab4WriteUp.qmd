...
df$evaluation.x1 = sapply(df$x1,evaluate.row)
```
...
#| size: "scriptsize"
#| code-fold: FALSE
df$evaluation.x1 = ifelse(df$x1 < 4, "low", ifelse(df$x1 <= 7, "mid", "high"))
```
...
#| size: "scriptsize"
#| code-fold: FALSE
url = "website/KimchiGrilledCheese.html"
(recipe.item = read_html(url))
```
...
#| size: "scriptsize"
#| code-fold: FALSE
(image.url = paste0("website/",html_attr(image.element,"src")))
```
...
all_files <- list.files("website/", pattern = "*.html")
recipe_files <- all_files[all_files != "about.html" & all_files != "index.html"]

set.seed(7272)
selected_meals <- sample(recipe_files, 4)

all_ingredients <- c()
recipe_list <- list()

for (i in 1:length(selected_meals)) {
  recipe_path <- paste0("website/", selected_meals[i])
  raw_html <- read_html(recipe_path)
  
  ingreds <- html_elements(raw_html, "#ingredients li")
  instruc <- html_elements(raw_html, "#instructions li")
  img_element <- html_element(raw_html, "img")
  img_src <- html_attr(img_element, "src")
  
  all_ingredients <- c(all_ingredients, html_text(ingreds))
  
  recipe_list[[i]] <- list(
    name = gsub(".html", "", selected_meals[i]),
    img = paste0("website/", img_src),
    ingredients = html_text(ingreds),
    instructions = html_text(instruc)
  )
}

cat("# Grocery List\n")
cat(paste("-", unique(all_ingredients)), sep = "\n")
cat("\n\\newpage\n")

for (i in 1:length(recipe_list)) {
  current_recipe <- recipe_list[[i]]
  
  cat("##", current_recipe$name, "\n")
  cat(paste0('<img src="', current_recipe$img, '" style="width: 50%; height: auto;">\n\n'))
  
  cat("### Ingredients\n")
  cat(paste("-", current_recipe$ingredients), sep = "\n")
  
  cat("\n### Instructions\n")
  cat(paste("1.", current_recipe$instructions), sep = "\n")
